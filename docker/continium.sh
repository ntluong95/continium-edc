...
# source template for the generated rustfs-init.sh in docker/continium.sh for
...
if [ -n "${RUSTFS_CORS_ALLOWED_ORIGINS:-}" ]; then
  ...
  cors_file="/tmp/continium-cors.xml"
...
cat > /tmp/continium-policy.json << EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {
...
if ! mc admin policy info rustfs "$RUSTFS_POLICY_NAME" >/dev/null 2>&1; then
  mc admin policy create rustfs "$RUSTFS_POLICY_NAME" /tmp/continium-policy.json || \
    mc admin policy add rustfs "$RUSTFS_POLICY_NAME" /tmp/continium-policy.json
...
write_rustfs_env_file() {
  ...
  upsert_dotenv_var "CONTINIUM_RUSTFS_ADMIN_USER" "$rustfs_admin_user" "$env_file"
  upsert_dotenv_var "CONTINIUM_RUSTFS_ADMIN_PASSWORD" "$rustfs_admin_password" "$env_file"
  upsert_dotenv_var "CONTINIUM_RUSTFS_SERVICE_USER" "$rustfs_service_user" "$env_file"
  upsert_dotenv_var "CONTINIUM_RUSTFS_SERVICE_PASSWORD" "$rustfs_service_password" "$env_file"
  upsert_dotenv_var "CONTINIUM_RUSTFS_BUCKET_NAME" "$rustfs_bucket_name" "$env_file"
  upsert_dotenv_var "CONTINIUM_RUSTFS_POLICY_NAME" "$rustfs_policy_name" "$env_file"
  upsert_dotenv_var "CONTINIUM_RUSTFS_REGION" "us-east-1" "$env_file"
}

install_continium() {
  # Friendly welcome
  echo "🧱 Welcome to the Continium Setup Script"
  echo ""
  echo "🛸 Fasten your seatbelts! We're setting up your Continium environment on your $ubuntu_version server."
  echo ""
  ...
  echo "🎉 Hooray! Docker is all set and ready to go. You're now ready to run your Continium instance!"

  mkdir -p continium && cd continium
  echo "📁 Created Continium Quickstart directory at ./continium."
  ...
  if [[ $https_setup == "n" ]]; then
    echo "You have chosen not to set up HTTPS certificate for your domain. Please make sure to set up HTTPS on your own. You can refer to the Continium documentation(https://continium.com/docs/self-hosting/custom-ssl) for more information."
...
EOT
  ...
  if [[ $configure_uploads == "y" ]]; then
    ...
    if [[ $use_external_s3 == "y" ]]; then
      ...
      rustfs_admin_user="continium-$(openssl rand -hex 4)"
      rustfs_admin_password=$(openssl rand -base64 20)
      rustfs_service_user="continium-service-$(openssl rand -hex 4)"
      rustfs_service_password=$(openssl rand -base64 20)
      rustfs_bucket_name="continium-uploads"
      rustfs_policy_name="continium-policy"
  ...
  echo "📥 Downloading docker-compose.yml from Continium GitHub repository..."
  curl -fsSL -o docker-compose.yml https://raw.githubusercontent.com/continium/continium/stable/docker/docker-compose.yml
  ...
  if [[ $use_external_s3 == "y" ]]; then
    ...
    sed -i 's|# S3_ACCESS_KEY:|S3_ACCESS_KEY: "${CONTINIUM_RUSTFS_SERVICE_USER}"|' docker-compose.yml
    sed -i 's|# S3_SECRET_KEY:|S3_SECRET_KEY: "${CONTINIUM_RUSTFS_SERVICE_PASSWORD}"|' docker-compose.yml
    sed -i 's|# S3_REGION:|S3_REGION: "${CONTINIUM_RUSTFS_REGION}"|' docker-compose.yml
    sed -i 's|# S3_BUCKET_NAME:|S3_BUCKET_NAME: "${CONTINIUM_RUSTFS_BUCKET_NAME}"|' docker-compose.yml
    if [[ $https_setup == "y" ]]; then
  ...
  # Step 1: Add Traefik labels to continium service
  awk -v domain_name="$domain_name" -v hsts_enabled="$hsts_enabled" '
/continium:/,/^ *$/ {
    if ($0 ~ /<<: \*environment$/) {
        print "    labels:"
        print "      - \"traefik.enable=true\""
        print "      - \"traefik.http.routers.continium.rule=Host(`" domain_name "`)\""
        print "      - \"traefik.http.routers.continium.entrypoints=websecure\""
        print "      - \"traefik.http.routers.continium.tls=true\""
        print "      - \"traefik.http.routers.continium.tls.certresolver=default\""
        print "      - \"traefik.http.services.continium.loadbalancer.server.port=3000\""
        if (hsts_enabled == "y") {
            ...
            print "      - \"traefik.http.routers.continium_http.entrypoints=web\""
            print "      - \"traefik.http.routers.continium_http.rule=Host(`" domain_name "`)\""
        }
        print $0
    } else {
        print $0
...
' docker-compose.yml >tmp.yml && mv tmp.yml docker-compose.yml
  ...
  # Step 2: Ensure continium waits for rustfs-init to complete successfully (mapping depends_on)
...
EOF
  ...
  rustfs:
    ...
    command: /data
    environment:
      RUSTFS_ACCESS_KEY: "\${CONTINIUM_RUSTFS_ADMIN_USER}"
      RUSTFS_SECRET_KEY: "\${CONTINIUM_RUSTFS_ADMIN_PASSWORD}"
      RUSTFS_ADDRESS: ":9000"
    volumes:
      - rustfs-data:/data
    labels:
...
EOF
  ...
  rustfs-init:
    ...
    depends_on:
      - rustfs
    environment:
      RUSTFS_ADMIN_USER: "\${CONTINIUM_RUSTFS_ADMIN_USER}"
      RUSTFS_ADMIN_PASSWORD: "\${CONTINIUM_RUSTFS_ADMIN_PASSWORD}"
      RUSTFS_SERVICE_USER: "\${CONTINIUM_RUSTFS_SERVICE_USER}"
      RUSTFS_SERVICE_PASSWORD: "\${CONTINIUM_RUSTFS_SERVICE_PASSWORD}"
      RUSTFS_BUCKET_NAME: "\${CONTINIUM_RUSTFS_BUCKET_NAME}"
      RUSTFS_POLICY_NAME: "\${CONTINIUM_RUSTFS_POLICY_NAME}"
...
echo "🔗 To edit more variables and deeper config, go to the continium/docker-compose.yml, edit the file, and restart the container!"
...
if [[ $rustfs_storage == "y" ]]; then
    ...
    echo "   • Generated credentials stored in ./continium/.env (permissions set to 600)"
    echo ""
fi

echo "🎉 All done! Please setup your Continium instance by visiting your domain at https://$domain_name. You can check the status of Continium & Traefik with 'cd continium && sudo docker compose ps.'"
...
uninstall_continium() {
  echo "🗑️ Preparing to Uninstalling Continium..."
  read -p "Are you sure you want to uninstall Continium? This will delete all the data associated with it! (yes/no): " uninstall_confirmation
  uninstall_confirmation=$(echo "$uninstall_confirmation" | tr '[:upper:]' '[:lower:]')
  if [[ $uninstall_confirmation == "yes" ]]; then
    cd continium
    sudo docker compose down
    cd ..
    sudo rm -rf continium
    echo "🛑 Continium uninstalled successfully!"
  else
    echo "❌ Uninstalling Continium has been cancelled."
  fi
}

stop_continium() {
  echo "🛑 Stopping Continium..."
  cd continium
  sudo docker compose down
  echo "🎉 Continium instance stopped successfully!"
}

update_continium() {
  echo "🔄 Updating Continium..."
  cd continium
  sudo docker compose pull
  sudo docker compose down
  sudo docker compose up -d
  echo "🎉 Continium updated successfully!"
  echo "🎉 Check the status of Continium & Traefik with 'cd continium && sudo docker compose logs.'"
}

restart_continium() {
  echo "🔄 Restarting Continium..."
  cd continium
  sudo docker compose restart
  echo "🎉 Continium restarted successfully!"
}

get_logs() {
  echo "📃 Getting Continium logs..."
  cd continium
...
if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
  ...
  uninstall)
    ...
    ;;
  *)
    echo "🚀 Executing default step of installing Continium"
    install_continium
    ;;
  esac
fi
