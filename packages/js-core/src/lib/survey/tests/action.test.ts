...
describe("survey/action.ts", () => {
  ...
  describe("trackAction", () => {
    test("logs debug message with action name", async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith('Continium: Action "testAction" tracked');
    });
    ...
    test("logs debug message with alias if provided", async () => {
      ...
      expect(result.ok).toBe(true);
      expect(mockLogger.debug).toHaveBeenCalledWith('Continium: Action "aliasName" tracked');
    });
  ...
  describe("trackCodeAction", () => {
    ...
    test("tracks action when code is valid", async () => {
      ...
      expect(result.ok).toBe(true);
      expect(mockLogger.debug).toHaveBeenCalledWith('Continium: Action "valid_code" tracked');
    });
  });
  ...
  describe("trackNoCodeAction", () => {
    test("calls trackAction with provided name", async () => {
      ...
      expect(result.ok).toBe(true);
      expect(mockLogger.debug).toHaveBeenCalledWith('Continium: Action "noCodeAction" tracked');
    });
  });
});
