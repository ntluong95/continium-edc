...
vi.mock("@/lib/common/config", () => ({
  ...
  CONTAINER_ID: "continium-container",
  RN_ASYNC_STORAGE_KEY: "continium-react-native",
}));
...
describe("widget-file", () => {
  ...
  beforeEach(() => {
    ...
    delete window.continiumSurveys;
  ...
  test("renderWidget sets isSurveyRunning, handles delay, loads continiumSurveys, and calls .renderSurvey", async () => {
    ...
    // @ts-expect-error -- mock window.continiumSurveys
    window.continiumSurveys = {
      renderSurvey: vi.fn(),
    };
    ...
    expect(window.continiumSurveys.renderSurvey).toHaveBeenCalledWith(
  ...
  test("closeSurvey removes widget container, resets filtered surveys, sets isSurveyRunning=false", () => {
    ...
    document.body.innerHTML = `<div id="continium-container"></div>`;
    widget.closeSurvey();
    expect(document.getElementById("continium-container")).toBeFalsy();

    expect(mockConfigValue.update).toHaveBeenCalled();
  });

  test("addWidgetContainer creates #continium-container in DOM", () => {
    expect(document.getElementById("continium-container")).toBeFalsy();
    widget.addWidgetContainer();
    const el = document.getElementById("continium-container");
    expect(el).not.toBeNull();
  });

  test("removeWidgetContainer removes #continium-container if it exists", () => {
    document.body.innerHTML = `<div id="continium-container"></div>`;
  ...
  test("renderWidget does not wait when no identification is pending", async () => {
    ...
    vi.advanceTimersByTime(0);
    expect(window.continiumSurveys.renderSurvey).toHaveBeenCalled();
  ...
  test("renderWidget skips survey when identification fails and survey has segment filters", async () => {
    ...
    expect(window.continiumSurveys.renderSurvey).not.toHaveBeenCalled();
  });

  describe("loadContiniumSurveysExternally and waitForSurveysGlobal", () => {
    ...
    test("rejects when script fails to load (onerror) and allows retry", async () => {
      ...
      await renderPromise;
      expect(consoleSpy).toHaveBeenCalledWith("Failed to load Continium Surveys library:", "Network error");
    ...
    test("rejects when script loads but surveys global never becomes available (timeout)", async () => {
      ...
      // Script loaded but window.continiumSurveys is never set
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "Failed to load Continium Surveys library:",
        expect.any(Error)
      );
    ...
    test("resolves after polling when surveys global becomes available and applies stored nonce", async () => {
      ...
      window.__continiumNonce = "test-nonce-123";
      ...
      // @ts-expect-error -- mock window.continiumSurveys
      window.continiumSurveys = { renderSurvey: vi.fn(), setNonce: vi.fn() };
      ...
      expect(window.continiumSurveys.setNonce).toHaveBeenCalledWith("test-nonce-123");
      expect(window.continiumSurveys.renderSurvey).toHaveBeenCalledWith(
        expect.objectContaining({
          appUrl: "https://fake.app",
          environmentId: "env_123",
      ...
      vi.useRealTimers();
      delete window.__continiumNonce;
    });
    ...
    test("deduplicates concurrent calls (returns cached promise)", async () => {
      ...
      // Calling renderWidget again (without continiumSurveys on window, but with cached promise)
