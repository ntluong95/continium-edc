...
export const trackAction = async (
  ...
  logger.debug(`Continium: Action "${aliasName}" tracked`);
...
export const trackCodeAction = async (
  ...
  if (!actionClass) {
    return err({
      code: "invalid_code",
      message: `Action with identifier '${code}' is unknown. Please add this action in Continium in order to use it via the SDK action tracking.`,
    });
  }
