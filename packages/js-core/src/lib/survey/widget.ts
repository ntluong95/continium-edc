...
export const renderWidget = async (
  ...
  let continiumSurveys: TContiniumSurveys;
  try {
    continiumSurveys = await loadContiniumSurveysExternally();
  } catch (error) {
...
type TContiniumSurveys = NonNullable<typeof globalThis.window.continiumSurveys>;

let surveysLoadPromise: Promise<TContiniumSurveys> | null = null;

const waitForSurveysGlobal = (): Promise<TContiniumSurveys> => {
  return new Promise((resolve, reject) => {
    ...
    const check = (): void => {
      ...
      if (globalThis.window.continiumSurveys) {
        const storedNonce = globalThis.window.__continiumNonce;
        if (storedNonce) {
          globalThis.window.continiumSurveys.setNonce(storedNonce);
        }
        resolve(globalThis.window.continiumSurveys);
        return;
      }

      if (Date.now() - startTime >= SURVEYS_LOAD_TIMEOUT_MS) {
        reject(new Error("Continium Surveys library did not become available within timeout"));
        return;
      }
...
const loadContiniumSurveysExternally = (): Promise<TContiniumSurveys> => {
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- Runtime check for surveys package availability
  if (globalThis.window.continiumSurveys) {
    return Promise.resolve(globalThis.window.continiumSurveys);
  }
  ...
  surveysLoadPromise = new Promise<TContiniumSurveys>((resolve, reject: (error: unknown) => void) => {
    const config = Config.getInstance();
    ...
    script.onload = () => {
      waitForSurveysGlobal()
        ...
        .catch((error: unknown) => {
          ...
          console.error("Failed to load Continium Surveys library:", error);
          reject(new Error(`Failed to load Continium Surveys library`));
        });
    };
...
export const preloadSurveysScript = (appUrl: string): void => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- Runtime check for surveys package availability
  if (globalThis.window.continiumSurveys) return;
