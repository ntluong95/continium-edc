...
export const setup = async (
  ...
  // continium is in error state, skip setup
  if (existingConfig?.status.value === "error") {
    if (isDebug) {
      logger.debug(
        "Continium is in error state, but debug mode is active. Resetting config and continuing."
      );
      config.resetConfig();
      return okVoid();
    }

    console.error("🧱 Continium - Continium was set to an error state.");

    const expiresAt = existingConfig.status.expiresAt;

    if (expiresAt && !isNowExpired(new Date(expiresAt))) {
      console.error("🧱 Continium - Error state is not expired, skipping initialization");
      return okVoid();
    }
    console.error("🧱 Continium - Error state is expired. Continuing with initialization.");
  }
...
export const handleErrorOnFirstSetup = (e: { code: string; responseMessage: string }): Promise<never> => {
  ...
  // put continium in error state (by creating a new config) and throw error
  ...
  throw new Error("Could not set up continium");
};

export const putContiniumInErrorState = (continiumConfig: Config): void => {
  const logger = Logger.getInstance();

  if (getIsDebug()) {
    logger.debug("Not putting continium in error state because debug mode is active (no error state)");
    return;
  }

  logger.debug("Putting continium in error state");
  // change continium status to error
  continiumConfig.update({
    ...continiumConfig.get(),
    status: {
      value: "error",
      expiresAt: new Date(new Date().getTime() + 10 * 60000), // 10 minutes in the future
