  ...
  private async run(): Promise<void> {
    ...
    while (this.queue.length > 0) {
      ...
      if (currentItem.checkSetup) {
        ...
        if (!setupResult.ok) {
          console.warn(`🧱 Continium - Setup not complete.`);
          continue;
        }
      }
      ...
      if (currentItem.type === CommandType.GeneralAction) {
        ...
        if (!updateQueue.isEmpty()) {
          console.log("🧱 Continium - Waiting for pending updates to complete before executing command");
      ...
      if (!result.ok) {
        console.error("🧱 Continium - Global error: ", result.error);
      } else if (!result.data.ok) {
        console.error("🧱 Continium - Global error: ", result.data.error);
      }
    }
