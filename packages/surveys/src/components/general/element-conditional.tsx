...
import { type TResponseData, type TResponseDataValue, type TResponseTtc } from "@continium/types/responses";
...
import { type TSurveyElement, type TSurveyElementChoice } from "@continium/types/surveys/elements";
...
export function ElementConditional({
  ...
  useEffect(() => {
    if (!isRecognizedType) {
      console.warn(
        `[Continium] Unrecognized element type "${element.type}" for element with id "${element.id}". No component will be rendered.`
      );
    }
