...
import { ApiErrorResponse } from "@continium/types/errors";
import { TQuotaFullResponse } from "@continium/types/quota";
import { TResponseUpdate } from "@continium/types/responses";
...
export class ResponseQueue {
  ...
  private async sendResponseWithRetry(responseUpdate: TResponseUpdate): Promise<{
    ...
    while (attempts < this.config.retryAttempts) {
      ...
      if (res.ok) {
        ...
        if (attempts > 0) {
          console.log(`Continium: Response sent successfully after ${attempts + 1} attempts`);
        }
      ...
      if (this.isRecaptchaError(res.error)) {
        console.error("Continium: Recaptcha verification failed", {
          error: res.error,
          responseId: this.surveyState.responseId,
        });
      ...
      console.error(`Continium: Response send failed`, {
        attempt: attempts + 1,
    ...
    console.error(`Continium: Failed to send response after ${this.config.retryAttempts} attempts`, {
      queueLength: this.queue.length,
      responseId: this.surveyState.responseId,
      surveyId: this.surveyState.surveyId,
    });
  ...
  async sendResponse(
    ...
    try {
      ...
      console.error("Continium: Error sending response", error);
      return err({
