...
export const renderSurvey = (props: SurveyContainerProps) => {
  ...
  if (mode === "inline") {
    ...
    modalContainer.id = "continium-modal-container";
...
// Initialize the global continiumSurveys object if it doesn't exist
if (globalThis.window !== undefined) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Type definition is in @continium/types package
  (globalThis.window as any).continiumSurveys = {
    renderSurveyInline,
    renderSurveyModal,
    renderSurvey,
    onFilePick,
    setNonce: setStyleNonce,
  } as typeof globalThis.window.continiumSurveys;
}
