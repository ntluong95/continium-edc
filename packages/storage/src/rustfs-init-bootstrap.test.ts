...
const continiumScriptPath = fileURLToPath(new URL("../../../docker/continium.sh", import.meta.url));
const rustfsInitTemplatePath = fileURLToPath(new URL("../../../docker/rustfs-init.sh", import.meta.url));

const tempDirs: string[] = [];

const createTempDir = (): string => {
  const tempDir = mkdtempSync(join(tmpdir(), "continium-rustfs-init-"));
...
if [ "$1" = "mb" ] && [ "$2" = "rustfs/continium" ] && [ "$3" = "--ignore-existing" ]; then
  exit 0
fi

if [ "$1" = "cors" ] && [ "$2" = "set" ] && [ "$3" = "rustfs/continium" ]; then
...
const writeRustfsInitScript = (targetPath: string): void => {
  execFileSync(
    "bash",
    ["-lc", 'source "$1"; write_rustfs_init_script "$2"', "bash", continiumScriptPath, targetPath],
    { encoding: "utf8" }
  );
};
...
afterEach(() => {
  ...
  rmSync("/tmp/continium-cors.xml", { force: true });
  rmSync("/tmp/continium-policy.json", { force: true });
});

describe("docker/continium.sh RustFS bootstrap", () => {
  ...
  test("generated init script provisions a bucket-scoped policy for the service user", () => {
    ...
    execFileSync(generatedScriptPath, {
      ...
      env: {
        ...
        RUSTFS_BUCKET_NAME: "continium",
        RUSTFS_POLICY_NAME: "continium-app-policy",
    ...
    expect(mcCalls).toEqual([
      ...
      "ls rustfs",
      "mb rustfs/continium --ignore-existing",
      "cors set rustfs/continium /tmp/continium-cors.xml",
      "admin policy info rustfs continium-app-policy",
      "admin policy create rustfs continium-app-policy /tmp/continium-policy.json",
      "admin user info rustfs service-user",
      "admin user add rustfs service-user service-password",
      "admin policy attach rustfs continium-app-policy --user service-user",
    ]);
...
`);
  ...
  test("generated init script falls back to policy add when policy create is unavailable", () => {
    ...
    expect(mcCalls).toContain("admin policy create rustfs continium-app-policy /tmp/continium-policy.json");
    expect(mcCalls).toContain("admin policy add rustfs continium-app-policy /tmp/continium-policy.json");
  });
