...
The database package for the Continium monorepo, providing centralized database schema management, migration handling, and type definitions for the entire platform.

## Overview

This package serves as the central database layer for Continium, containing:
...
### Important: do NOT run `prisma migrate deploy` directly

The canonical migration path is the custom runner at `src/scripts/migration-runner.ts`, invoked via `pnpm db:migrate:deploy` (prod) or `pnpm db:migrate:dev` (local). The runner:

1. Discovers migrations under `migration/` (NOT `prisma/migrations/`).
2. Interleaves schema migrations with data migrations in timestamp order.
3. Tracks schema migrations in Prisma's `_prisma_migrations` table and data migrations in `DataMigration`.
4. Copies one schema migration at a time into the temp `migrations/` directory before invoking `prisma migrate deploy` internally.

Running stock `prisma migrate deploy` directly against a Continium database bypasses the data-migration steps and the migration-history reconciliation. This will leave the schema in an inconsistent state.

### Timestamp uniqueness

Each migration directory must have a unique 14-digit timestamp prefix. Two directories with the same timestamp produce a nondeterministic apply order (alphabetical fallback differs across filesystems). The CI guard `pnpm db:migrations:check-timestamps` enforces this.

### Schema vs DB parity

Drift between `schema.prisma` and the live database (missing tables, mismatched enum values) is the most common class of bug in this package. The `pnpm db:check:schema-parity` script connects to the database via `PARITY_DATABASE_URL` (falling back to `DATABASE_URL`) and reports:

- Models declared in schema but absent from the DB
- Enums declared in schema but absent from the DB
- Enum value mismatches in either direction

Run this against a freshly migrated DB on every PR that touches `packages/database/`.
...
Run these commands from the root directory of the Continium monorepo:
...
```json
{
  ...
  "db:partitions:lint": "CI: enforce PARTITION BY on partitioned tables",
  "db:migrations:check-timestamps": "CI: fail when two migrations share a timestamp",
  "db:check:schema-parity": "CI: assert schema.prisma matches the live DB"
}
```
...
2. **Users**: Creates default users with the following credentials (passwords are hashed):
   - **Admin**: `admin@continium.com` / `password123`
   - **Manager**: `manager@continium.com` / `password123`
...
- **Models**: Comprehensive schema covering users, organizations, surveys, responses, contacts, and more
...
For more information about the Continium project structure, see the main repository README.
