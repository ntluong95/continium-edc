charts/continium is a Helm chart for deploying Continium to Kubernetes.

It is not part of the main app logic
...
A Helm chart for Continium with PostgreSQL, Redis

**Homepage:** <https://continium.com/docs/self-hosting/setup/kubernetes>

## Maintainers

| Name       | Email                 | Url |
| ---------- | --------------------- | --- |
| Continium | <info@continium.com> |     |
...
The chart can optionally deploy Continium behind Envoy Gateway with a dedicated Redis HA backend for Envoy global
rate limiting.
...
- OpenTelemetry proxy metrics can target either `envoy.continium.proxy.telemetry.openTelemetry.host`/`port`
  or `backendRefs`, but not both. Prefer `host`/`port` for collectors that live in another namespace.
- When both the main app ingress and the Envoy API ingress are enabled, set `envoy.continium.ingress.host`
...
| deployment.image.repository                                        | string | `"ghcr.io/continium/continium"` |             |
...
| ingress.hosts[0].host                                              | string | `"k8s.continium.com"`            |             |
| ingress.hosts[0].paths[0].path                                     | string | `"/"`                             |             |
| ingress.hosts[0].paths[0].pathType                                 | string | `"Prefix"`                        |             |
| ingress.hosts[0].paths[0].serviceName                              | string | `"continium"`                    |             |
| ingress.ingressClassName                                           | string | `"alb"`                           |             |
| nameOverride                                                       | string | `""`                              |             |
| partOfOverride                                                     | string | `""`                              |             |
| postgresql.auth.database                                           | string | `"continium"`                    |             |
| postgresql.auth.existingSecret                                     | string | `"continium-app-secrets"`        |             |
...
| postgresql.auth.username                                           | string | `"continium"`                    |             |
| postgresql.enabled                                                 | bool   | `true`                            |             |
| postgresql.externalDatabaseUrl                                     | string | `""`                              |             |
| postgresql.fullnameOverride                                        | string | `"continium-postgresql"`         |             |
...
| redis.auth.existingSecret                                          | string | `"continium-app-secrets"`        |             |
| redis.auth.existingSecretPasswordKey                               | string | `"REDIS_PASSWORD"`                |             |
| redis.enabled                                                      | bool   | `true`                            |             |
| redis.externalRedisUrl                                             | string | `""`                              |             |
| redis.fullnameOverride                                             | string | `"continium-redis"`              |             |
