...
description: "Overview of the Continium database schema and relationships"
icon: "database"
---

Continium uses PostgreSQL as its primary database and [Prisma](https://www.prisma.io/) as the Object-Relational Mapping (ORM) tool. The database schema is designed to support multi-tenancy, survey management, and response collection while maintaining data isolation between organizations.

## Entity Relationship Diagram

The following diagram shows the core entities and their relationships in the Continium database:
...
2. **Project**
   ...
   - **kind**: Distinguishes product-survey projects (`PRODUCT`, default) from regulated clinical EDC projects (`CLINICAL`). One-way toggle in v1 (PRODUCT → CLINICAL only).
...
The `AuditLog` table provides a compliance audit trail required by 21 CFR Part 11 and ICH-GCP regulations.

**Key Properties:**
- **Append-Only**: Immutable via database trigger (BEFORE UPDATE/DELETE raises exception)
- **Monthly Partitioning**: `audit_log` table partitioned by RANGE on `occurredAt` for scalability
- **Indexed by Actor & Resource**: Composite indexes on `(projectId, occurredAt)` and `(actorId, occurredAt)` for fast lookups
- **Event Typing**: 52+ typed events (e.g., `RECORD_CREATED`, `USER_LOGIN`) plus `PRISMA_OPERATION` catch-all for middleware instrumentation
- **Metadata Capture**: Actor IP, user agent, resource ID, and optional diff JSON for updates
- **Async Persistence**: Enqueued via pg-boss job (`audit.write`) from Prisma middleware; sync fallback if worker unavailable

**Audit Event Examples:**
- User actions: `USER_REGISTERED`, `USER_LOGIN`, `USER_LOGIN_FAILED`
- Record lifecycle: `RECORD_CREATED`, `RECORD_UPDATED`, `RECORD_LOCKED`, `RECORD_SIGNED`
- Operational changes: `FORM_UPDATED`, `METADATA_PUBLISHED`, `DAG_ASSIGNED`
- Access control: `PERMISSION_DENIED`, `MEMBERSHIP_REVOKED`, `ROLE_DELETED`

See `docs/development/technical-handbook/background-job-processing.mdx` for async job details.
...
For detailed information about specific models and their properties, refer to the [schema.prisma](https://github.com/continium/continium/blob/main/packages/database/schema.prisma) file in the repository.

## PostgreSQL Database Tables

Continium stores all data in PostgreSQL tables. Here's a comprehensive list of all database tables and their purposes:
...
| AuditLog              | Append-only compliance audit trail (monthly RANGE partitions, immutable) |
...
| Project               | Groups related surveys and environments; supports PRODUCT or CLINICAL kind |
...
Please refer to the [Prisma schema file](https://github.com/continium/continium/blob/main/packages/database/schema.prisma) in our GitHub repository.
