...
description: "Standards for consistent code formatting across the Continium project"
icon: code
---
...
### Prettier Config
Continium uses a standardized Prettier configuration defined in `@continium/config-prettier`:
```javascript
module.exports = {
  bracketSpacing: true,
...
- Consistent use of `tsconfig.json` extending from `@continium/config-typescript`
...
```json
{
  ...
  "extends": "@continium/config-typescript/js-library.json",
...
Each package extends from `@continium/eslint-config` with specific presets:
...
These standards ensure consistency across the Continium codebase while maintaining high code quality and developer productivity.
