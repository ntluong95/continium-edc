...
description: This document outlines the language-specific conventions for the Continium codebase, providing guidelines for writing code in TypeScript/JavaScript.
icon: code
---
...
2. **Architecture Patterns**
   ...
   - **Singleton Pattern**: Main SDK class (`Continium`) with static interface
   - **Manager Pattern**: Specialized managers (`UserManager`, `SurveyManager`, `PresentSurveyManager`)
   - **Builder Pattern**: Configuration objects (`ContiniumConfig.Builder`)
   - **Protocol-Oriented Programming**: Service protocols for dependency injection and testing

3. **Error Handling**

   - Custom error enums with descriptive cases (`ContiniumSDKErrorType`)
...
4. **Naming Conventions**
   ...
   - Classes: PascalCase (`ContiniumConfig`, `UserManager`)
   - Properties/Methods: camelCase (`environmentId`, `setUserId`)
   - Constants: camelCase with descriptive names
   - Protocol names: Descriptive with "Protocol" suffix (`ContiniumServiceProtocol`)
...
```swift
// Class definitions
@objc(Continium) public class Continium: NSObject {
...
@objc public static func setup(with config: ContiniumConfig, force: Bool = false)

// Guard statements for early returns
guard !isInitialized else {
    let error = ContiniumSDKError(type: .sdkIsAlreadyInitialized)
    Continium.logger?.error(error.message)
    return
}

// Enum cases with descriptive names
public enum ContiniumSDKErrorType: Int {
...
2. **Kotlin Language Features**
   ...
   - **Object singletons** for stateless utilities and managers (`Continium`, `Logger`, `SDKError`)
