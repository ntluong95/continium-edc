...
This usually happens when the Continium Widget isn't correctly or completely built.

```bash
pnpm build --filter=@continium/js
...
```bash helloWorld.js
pnpm dev --filter=@continium/web...
```

It’s better to use a single terminal with `pnpm dev` rather than having multiple open (one with the Continium app and one with the demo).
