---
title: "HubSpot"
description: "Learn how to integrate Continium with HubSpot to automatically create or update contacts when survey responses are submitted."
---

<Note>
  Continium doesn't have a native HubSpot integration yet. This guide shows you how to connect Continium with HubSpot using automation platforms (Make.com, n8n) or custom webhooks.
</Note>
...
1. **A Continium account** with at least one survey that collects email addresses
...
<Steps>
  ...
  <Step title="Create the App">
    Click **Create a private app** and give it a name (e.g., "Continium Integration").
  </Step>
...
<Steps>
  ...
  <Step title="Create a New Make.com Scenario">
    Go to [Make.com](https://make.com) and create a new scenario. Search for **Continium** and select it as your trigger, then choose **Response Finished** as the trigger event.

    ![Search Continium](/images/xm-and-surveys/core-features/integrations/make/search-continium.webp)
  </Step>
  <Step title="Connect Continium to Make">
    Click **Create a webhook**, enter your Continium API Host (default: `https://app.continium.com`), add your Continium API Key (see [API Key Setup](/api-reference/rest-api#how-to-generate-an-api-key)), and select the survey you want to connect.
  ...
  <Step title="Add the HubSpot Module">
    Click the **+** button after the Continium trigger, search for **HubSpot**, choose **Create or Update a Contact** as the action, and connect your HubSpot account.
  </Step>
  <Step title="Map Continium Fields to HubSpot">
    Map the Continium response fields to HubSpot contact properties:

    | HubSpot Field | Continium Field |
    | ------------- | ---------------- |
  ...
  <Step title="Test and Activate">
    Submit a test response to your Continium survey, verify the contact appears in HubSpot, and turn on your Make scenario.
  </Step>
</Steps>
...
<Note>
  The Continium n8n node is available as a community node. Install it via **Settings** → **Community Nodes** → install `@continium/n8n-nodes-continium`.
</Note>

<Steps>
  <Step title="Set Up the Continium Trigger">
    Create a new workflow in n8n, add the **Continium** trigger node, connect it with your Continium API Key and host, select **Response Finished** as the event, and choose your survey.

    ![Add Continium Trigger](/images/xm-and-surveys/core-features/integrations/n8n/add-continium-trigger.webp)
  </Step>
  ...
  <Step title="Test Your Workflow">
    Click **Listen for event** in the Continium trigger, submit a test survey response, verify the data flows through to HubSpot, and activate your workflow.
  </Step>
</Steps>
...
For maximum flexibility, you can use Continium webhooks with a custom endpoint that calls the HubSpot API directly. This approach is ideal for developers who want full control.
...
<Steps>
  <Step title="Create a Continium Webhook">
    Go to **Configuration** → **Integrations** in Continium, click **Manage Webhooks** → **Add Webhook**, enter your endpoint URL, select **Response Finished** as the trigger, and choose the surveys to monitor.
  ...
  <Step title="Build Your Webhook Handler">
    ...
    - **Receive the Continium webhook** - Accept POST requests with the survey response payload
  ...
  <Step title="Deploy and Test">
    Deploy your webhook handler to a publicly accessible URL, add the URL to your Continium webhook configuration, submit a test survey response, and verify the contact appears in HubSpot.
  </Step>
</Steps>
...
Still struggling or something not working as expected? [Join our GitHub Discussions](https://github.com/continium/continium/discussions) and we're happy to help!
