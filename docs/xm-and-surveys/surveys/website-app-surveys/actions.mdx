---
title: "Actions"
description: "Actions are predefined events within your app that prompt Continium to display a survey when triggered. These are detected by the Continium widget, which then presents the appropriate survey based on your predefined settings."
icon: "code"
---
...
<Step title="Continium widget detects action">
...
<Step title="Check for survey trigger">
  If a survey is set to trigger on that action, Continium checks if the user or visitor qualifies.
</Step>
  ...
  <Step title="Check for user or visitor qualification">
    ...
    Also, if this user or visitor has seen this specific survey before, Continium might not show it as this is dependent on the [Recontact Options](/xm-and-surveys/surveys/website-app-surveys/recontact).
...
Continium offers an intuitive No-Code interface that allows you to configure actions without needing to write any code.
...
<Steps>
  <Step title="Visit the Configuration tab and click on the `Website & App Connection`">
    ![Action overview on Continium Open Source Survey Solution](/images/xm-and-surveys/surveys/website-app-surveys/actions/actions-view.webp "Action overview on Continium Open Source Survey Solution")
  </Step>
...
<Steps>
  <Step title="Configure action in Continium">
    First, add the action via the Continium web interface to make it available for survey configuration:
...
![Add a code action to open source in app survey](/images/xm-and-surveys/surveys/website-app-surveys/actions/code-action.webp "Add a code action to open source in app survey")
  ...
  <Step title="Add action tracking to your code">
    Use continium.track() to send an action event to Continium:

    ```
    continium.track("action");
    ```
    ...
    ```javascript
    const handleClick = () => {
      continium.track("Button Clicked");
    };
