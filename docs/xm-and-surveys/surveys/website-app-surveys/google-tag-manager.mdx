...
description: "Deploy Continium surveys through GTM without modifying your website code."
icon: "tags"
---
...
- Your Continium **Environment ID** (Settings > Configuration > Website & App Connection)
- Your **App URL**: `https://app.continium.com` (or your self-hosted URL)
...
<Steps>
  <Step title="Create a Custom HTML tag in GTM">
    1. Create a new tag with preferred name e.g. "Continium Intercept Surveys"
...
!function(){
    var appUrl = "https://app.continium.com"; // REPLACE ONLY IF YOUR SELF-HOST
    ...
    t.onload=function(){
        window.continium && window.continium.setup({
...
![Add GTM Custom HTML tag](/images/xm-and-surveys/surveys/website-app-surveys/google-tag-manager/create-a-tag.webp)
  ...
  <Step title="Test">
    ...
    3. Add `?continiumDebug=true` to the URL to see test logs in browser console (see [Debugging Mode](/xm-and-surveys/surveys/website-app-surveys/framework-guides#debugging-continium-integration) for more details)
  </Step>
</Steps>
...
<Note>
  User identification is part of the Continium [Enterprise Edition](/self-hosting/advanced/license). 
</Note>
...
<Steps>
  ...
  <Step title="Create identification tag">
    New Custom HTML tag named "Continium - User":

```html
<script>
(function() {
    var check = setInterval(function() {
        if (window.continium && window.continium.setUserId) {
            clearInterval(check);
            var userId = {{User ID}};
            if (userId) {
                window.continium.setUserId(userId);
                var attrs = {};
                if ({{User Email}}) attrs.email = {{User Email}};
                if ({{User Plan}}) attrs.plan = {{User Plan}};
                if (Object.keys(attrs).length) {
                    window.continium.setAttributes(attrs);
                }
            }
        }
    }, 100);
...
```
  ...
  <Step title="Set trigger and push data">
    ...
    4. Attach this trigger to your "Continium - User" tag
    5. Save and publish
...
<Steps>
  <Step title="Create code action in Continium">
    Add code action via Continium UI
...
```html
<script>
if (window.continium && window.continium.track) {
    window.continium.track({{Event Name}});
}
</script>
```
...
- Add `?continiumDebug=true` to your URL
...
**Events not tracking?**
- Confirm `window.continium` exists before calling track
- Match event names exactly with Continium action names
- Check timing - Continium must be initialized first

## Need Help?

- [GitHub Discussions](https://github.com/continium/continium/discussions)
