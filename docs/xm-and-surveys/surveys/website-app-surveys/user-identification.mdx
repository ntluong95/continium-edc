...
In Continium, understanding the difference between identified and unidentified users is crucial for effective survey segmentation and targeted feedback collection.
...
| Collect **user information** in Continium               | ❌                 | ✅               |
...
Identified users are those for whom specific information has been set, notably the userId. This identification allows for more precise targeting of surveys and a deeper understanding of the feedback provided. When enabled, all information specified by you and all actions are sent to Continium.

This method is recommended for applications where users are required to log in and will often return.

### Setting User ID

To enable user identification, call the `setUserId` function of Continium and pass the user id. The user will show up in the Continium dashboard. Use a unique string, like a database ID or a unique email address. You can also anonymize the identifier, as long as it is unique for each user.

```javascript
continium.setUserId("<user-id>");
```
...
continium.setAttribute("Plan", "free");
```

The `setAttribute` function works like this:

```javascript Setting Custom Attributes
continium.setAttribute("attribute_key", "attribute_value");
```
...
continium.setLanguage("de"); // ISO identifier or Alias set when creating language
```

### Logging Out Users

When a user logs out of your webpage, also log them out of Continium to prevent activity from being linked to the wrong user. Use the logout function:
