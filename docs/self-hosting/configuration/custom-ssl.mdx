...
description: "Using Continium One-Click Setup with a Custom SSL Certificate."
icon: "lock"
---

<Note>
  Continium One-Click setup already comes with a valid SSL certificate using **Let's Encrypt**. This guide is only if you already have a valid SSL certificate that you need to use due to company policy or other requirements.
</Note>

Continium' One-Click setup automatically creates an SSL certificate using **Let's Encrypt**. However, some setups need a custom SSL certificate. This is common for intranets or systems with special certificate rules that use an internal or custom certificate authority (CA).

### Step 1: Navigate to the Continium Folder

Move into the `continium/` directory:
  ...
  services:
    continium:
      ...
      labels:
        ...
        - "traefik.http.routers.continium.rule=Host(`my-domain.com`)"  # Use your actual domain or IP
        - "traefik.http.routers.continium.entrypoints=websecure"  # Use the websecure entrypoint (port 443 with TLS)
        - "traefik.http.routers.continium.tls=true"  # Enable TLS
        - "traefik.http.services.continium.loadbalancer.server.port=3000"  # Forward traffic to Continium on port 3000
      ports:
        - 3000:3000
      volumes:
...
* **Navigate to the Continium folder**
...
This setup ensures that Continium securely communicates using your own SSL certificate. 🚀
