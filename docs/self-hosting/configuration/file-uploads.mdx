...
description: "Configure file storage for survey images, file uploads, and project assets in your self-hosted Continium instance"
icon: "upload"
---

Continium requires S3-compatible storage for file uploads. You can use external cloud storage services or the bundled RustFS option for a self-hosted solution.

## Why Configure File Uploads?

Setting up file storage enables important features in Continium, including:
...
Continium supports S3-compatible storage with two main configurations:
...
RustFS provides a self-hosted S3-compatible storage solution that runs alongside Continium. This option:

- Runs in a Docker container alongside Continium
...
When using the Continium installation script, you'll be prompted to configure file uploads:
...
Choose this option for a self-hosted S3-compatible storage that runs alongside Continium:
...
- Generate separate RustFS admin and Continium service credentials
- Create the storage bucket
- Configure SSL certificates for the files subdomain
- Configure Traefik routing for the subdomain
- Store the generated RustFS credentials in `./continium/.env` with restricted permissions
...
S3_REGION=us-east-1
S3_BUCKET_NAME=my-continium-uploads
...
S3_REGION=nyc3
S3_BUCKET_NAME=my-continium-space
...
Continium uses the S3 [POST Object](https://docs.aws.amazon.com/AmazonS3/latest/API/RESTObjectPOST.html)
operation (presigned POST) for uploads. Your object storage provider must support this operation. Providers
that do not implement POST Object are not compatible with Continium uploads. For example, Backblaze B2's
S3‑compatible API currently does not support POST Object and therefore will not work with Continium file
uploads.
...
- **Bucket**: Automatically created `continium-uploads` bucket
...
🗄️  RustFS Storage Setup Complete:
   • Access Key: continium-service-a1b2c3d4
   • Bucket: continium-uploads (✅ automatically created)
```
...
services:
  continium:
    image: ghcr.io/continium/continium:latest
    environment:
...
The following IAM policy grants the minimum required permissions for Continium to function correctly. This policy is also used in the bundled RustFS integration:
...
2. **Restricted Write Access**: Only your Continium instance should be able to upload files
3. **CORS Configuration**: Allow requests from your Continium domain
...
```json
{
  "Statement": [
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::123456789012:user/continium-service"
      },
      "Resource": "arn:aws:s3:::your-bucket-name/*",
      "Sid": "AllowContiniumWrite"
    }
  ],
  "Version": "2012-10-17"
}
...
<Note>
  ...
  `arn:aws:iam::123456789012:user/continium-service` with the ARN of your IAM user. This policy allows public
  read access only to specific paths while restricting write access to your Continium service user.
</Note>

### S3 CORS Configuration

CORS (Cross-Origin Resource Sharing) must be configured on your S3 bucket to allow Continium to upload files using presigned POST URLs. Without proper CORS configuration, file uploads from the browser will fail.
...
<Note>
  For production environments, consider restricting `AllowedOrigins` to your specific Continium domain(s)
...
- Ship RustFS and Continium container logs into your normal logging and alerting stack; the bundled setup
...
5. We use S3 presigned URLs for uploads. Make sure your CORS policy allows presigned URL uploads; otherwise, uploads will fail.
   Some providers (e.g., Hetzner’s object storage) [require a specific CORS configuration](https://github.com/continium/continium/discussions/6641#discussioncomment-14574048).
...
```bash
# Check Continium logs
docker compose logs continium

# Check RustFS logs (if using bundled RustFS)
docker compose logs rustfs
```

For additional help, join the conversation on [GitHub Discussions](https://github.com/continium/continium/discussions).
