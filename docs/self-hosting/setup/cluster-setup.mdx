---
title: "Cluster Setup"
description: "How to set up Continium in a High-Availability Cluster"
icon: "circle-nodes"
---

## Overview

Running Continium as a cluster of multiple instances offers several key advantages:
...
To run Continium in a cluster setup, you'll need:
...
The Continium cluster setup consists of multiple components working together to provide a scalable and highly available system. Here's a detailed overview of the architecture:
...
graph TD
    ...
    subgraph Continium Cluster
        FB1[Continium Instance 1]
        FB2[Continium Instance 2]
        FB3[Continium Instance n]
    end
...
1. **Continium Cluster**
   - Multiple Continium instances (1..n) running in parallel
...
5. **Load Balancer**
   - Distributes incoming traffic across all Continium instances
...
<Note>
  Redis is required for Continium to function. The application will not start without a Redis URL configured.
</Note>
...
- All Continium instances have access to the same S3 bucket
- The bucket has appropriate CORS settings configured
- IAM roles/users have sufficient permissions for read/write operations

## Kubernetes Setup

Continium provides an official Helm chart for deploying the entire cluster stack on Kubernetes. The Helm chart is available in the [Continium GitHub repository](https://github.com/continium/continium/tree/main/helm-chart).

### Features of the Helm Chart

The Helm chart provides a complete deployment solution that includes:

- Continium application with configurable replicas
...
1. Add the Continium Helm repository:

```sh
helm repo add continium https://raw.githubusercontent.com/continium/continium/main/helm-chart
helm repo update
```
...
```sh
helm install continium continium/continium
```
...
Refer to the [Helm chart documentation](https://github.com/continium/continium/tree/main/helm-chart) for detailed configuration options and examples.
