...
```sh
helm install continium oci://ghcr.io/continium/helm-charts/continium -n continium --create-namespace
```
...
```sh
helm install continium oci://ghcr.io/continium/helm-charts/continium -n continium --create-namespace --set enterprise.licenseKey="YOUR_LICENSE_KEY"
```
</Step>
</Steps>
...
Install with:
```sh
helm install continium oci://ghcr.io/continium/helm-charts/continium -n continium --create-namespace -f values.yaml
```
...
To apply changes:
```sh
helm upgrade continium oci://ghcr.io/continium/helm-charts/continium -n continium
```

### Scaling Resources
```sh
helm upgrade continium oci://ghcr.io/continium/helm-charts/continium -n continium   --set deployment.resources.limits.cpu=2   --set deployment.resources.limits.memory=4Gi
```

### Enabling Autoscaling
```sh
helm upgrade continium oci://ghcr.io/continium/helm-charts/continium -n continium   --set autoscaling.enabled=true   --set autoscaling.minReplicas=3   --set autoscaling.maxReplicas=10
```
...
| `deployment.image.repository` | Docker image repository              | `"ghcr.io/continium/continium"` |
...
```sh
helm uninstall continium -n continium
```
...
```sh
kubectl delete pvc --all -n continium
```

To completely delete the namespace:
```sh
kubectl delete namespace continium
```
...
**Your Continium deployment is now ready!**
