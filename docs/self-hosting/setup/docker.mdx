---
title: "Docker Setup"
description: "Set up Continium quickly using our ready-to-use Docker image."
icon: "docker"
---
...
1. **Create a New Directory for Continium**

   Open a terminal and run the following commands to create and enter a new directory for Continium:

   ```bash
   mkdir continium-quickstart && cd continium-quickstart
   ```

1. **Download the Docker-Compose File**

   Get the docker-compose file from the Continium repository by running:

   ```bash
   curl -o docker-compose.yml https://raw.githubusercontent.com/continium/continium/stable/docker/docker-compose.yml
   ```
...
1. **Start the Docker Setup**
   ...
   Now, you're ready to run Continium with Docker. Use the command below to start Continium along with a PostgreSQL database using Docker Compose:
...
1. **Open Continium in Your Browser**

   Once the setup is running, open [**http://localhost:3000**](http://localhost:3000) in your browser to access Continium. The first time you visit, you'll see a setup wizard. Follow the steps to create your first user and start using Continium.

## Update

Please take a look at our [migration guide](/self-hosting/advanced/migration) for version specific steps to update Continium.

1. Pull the latest Continium image

   ```bash
   docker compose pull
   ```

1. Stop the Continium stack

   ```bash
   docker compose down
   ```

1. Re-start the Continium stack with the updated image
...
RustFS provides S3-compatible object storage for file uploads in Continium. If you want to enable features
like image uploads, survey file uploads, or custom logos, you can run RustFS alongside Continium while
...
starts RustFS and auto-creates the `continium` bucket.
...
4. **Verify uploads**
   ...
   After uploading a file in Continium, open http://localhost:9001 and navigate to **Buckets → continium**
...
```bash
CONTINIUM_RUSTFS_ADMIN_USER=continium-root
CONTINIUM_RUSTFS_ADMIN_PASSWORD=change-this-secure-password
CONTINIUM_RUSTFS_SERVICE_USER=continium-service
CONTINIUM_RUSTFS_SERVICE_PASSWORD=change-this-service-password
CONTINIUM_RUSTFS_BUCKET_NAME=continium
CONTINIUM_RUSTFS_POLICY_NAME=continium-policy
CONTINIUM_RUSTFS_REGION=us-east-1
```

Then configure Continium to use the RustFS service credentials:

```bash
S3_ACCESS_KEY="${CONTINIUM_RUSTFS_SERVICE_USER}"
S3_SECRET_KEY="${CONTINIUM_RUSTFS_SERVICE_PASSWORD}"
S3_REGION="${CONTINIUM_RUSTFS_REGION}"
S3_BUCKET_NAME="${CONTINIUM_RUSTFS_BUCKET_NAME}"
...
- **Health check**: From the Continium container:

  ```bash
  docker compose exec continium sh -c 'wget -O- http://rustfs:9000/health'
  ```
...
- CORS configuration scoped to your Continium domain
- Rate limiting middleware
- Separate RustFS admin and Continium service credentials
- A `rustfs-init` job that creates the bucket and access policy

The production setup from [continium.sh](https://github.com/continium/continium/blob/main/docker/continium.sh)
...
<Note>
  ...
  storage or a dedicated RustFS deployment managed separately from Continium.
</Note>
...
```bash
[+] Running 9/16
⠹ continium 15 layers [⣿⣤⣿⣿⣿⣿⣿⣿⣿⣿⠀⠀⠀⠀⠀] 29.78MB/47.76MB Pulling                                                           13.3s
...
[+] Running 2/2
✔ Container continium-quickstart-postgres-1    Created                                                                       0.0s
✔ Container continium-quickstart-continium-1  Created                                                                       0.0s
```

And at the tail of the output, you should see this:

```bash
continium-quickstart-continium-1  | All migrations have been successfully applied.
continium-quickstart-continium-1  |
continium-quickstart-continium-1  | - info Loaded env from /home/nextjs/apps/web/.env
continium-quickstart-continium-1  | Listening on port 3000 url: http://<random-string>:3000
```
