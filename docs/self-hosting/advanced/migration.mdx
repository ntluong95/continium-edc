---
title: "Migration"
description: "Continium Self-hosted version migration"
icon: "arrow-right"
---

## v4.7

Continium v4.7 introduces **typed contact attributes** with native `number` and `date` data types. This enables comparison-based segment filters (e.g. "signup date before 2025-01-01") that were previously not possible with string-only attribute values.

### What Happens Automatically

When Continium v4.7 starts for the first time, the data migration will:
...
<Tabs>
  <Tab title="Docker">
    Before running these steps, navigate to the `continium` directory where your `docker-compose.yml` file is located.

    ```bash
    docker exec continium-postgres-1 pg_dump -Fc -U postgres -d continium > continium_pre_v4.7_$(date +%Y%m%d_%H%M%S).dump
    ```

    <Info>
      If you run into "**No such container**", use `docker ps` to find your container name, e.g.
      `continium_postgres_1`.
    </Info>
  ...
  <Tab title="Kubernetes">
    ...
    ```bash
    kubectl exec -n continium continium-postgresql-0 -- pg_dump -Fc -U continium -d continium > continium_pre_v4.7_$(date +%Y%m%d_%H%M%S).dump
    ```

    <Info>
      If your PostgreSQL pod has a different name, run `kubectl get pods -n continium` to find it.
    </Info>
...
<Tabs>
  ...
  <Tab title="Kubernetes">
    ```bash
    helm upgrade continium oci://ghcr.io/continium/helm-charts/continium \
      -n continium \
...
After Continium starts, check the logs to see whether the value backfill was completed or skipped:

<Tabs>
  <Tab title="Docker">
    ```bash
    docker compose logs continium | grep -i "backfill"
    ```
  </Tab>
  <Tab title="Kubernetes">
    ```bash
    # Check the application pod logs
    kubectl logs -n continium -l app.kubernetes.io/name=continium --tail=200 | grep -i "backfill"
    ```

    If the Helm migration Job ran, you can also inspect its logs:

    ```bash
    kubectl logs -n continium job/continium-migration
    ```
...
If the migration skipped the value backfill, run the standalone backfill script inside the running Continium container:

<Tabs>
  <Tab title="Docker">
    ```bash
    docker exec continium node packages/database/dist/scripts/backfill-attribute-values.js
    ```

    <Info>Replace `continium` with your actual container name if it differs. Use `docker ps` to find it.</Info>

  </Tab>
  <Tab title="Kubernetes">
    ```bash
    kubectl exec -n continium deploy/continium -- node packages/database/dist/scripts/backfill-attribute-values.js
    ```

    <Info>
      If your Continium deployment has a different name, run `kubectl get deploy -n continium` to find it.
    </Info>
...
- **Safe to run while Continium is live** -- it does not lock the entire table or wrap work in a long transaction
...
- Access your Continium instance at the same URL as before
...
Continium 4 introduces additional requirements for self-hosting setups and makes a dedicated Redis cache as well as S3-compatible file storage mandatory.

</Warning>

Continium 4.0 is a **major milestone** that sets up the technical foundation for future iterations and feature improvements. This release focuses on modernizing core infrastructure components to improve reliability, scalability, and enable advanced features going forward.

### What's New in Continium 4.0
...
These improvements in Continium 4.0 also make some infrastructure requirements mandatory going forward:
...
Additional migration steps are needed if you are using a self-hosted Continium setup that uses either local file storage (not S3-compatible file storage) or doesn't already use a Redis cache.
...
curl -fsSL -o migrate-to-v4.sh \
  https://raw.githubusercontent.com/continium/continium/stable/docker/migrate-to-v4.sh
...
- Pulls the latest Continium image and updates your instance

### Manual Setup

If you use a different setup to host your Continium instance, you need to make sure to make the necessary adjustments to run Continium 4.0.

#### Redis

Continium 4.0 requires a Redis instance to work properly. Please add a Redis instance to your Docker setup, your K8s infrastructure, or however you are hosting Continium at the moment. Continium works with the latest versions of Redis as well as Valkey.

You need to configure the `REDIS_URL` environment variable and point it to your Redis instance.

#### S3-compatible storage

To use file storage (e.g., file upload questions, image choice questions, custom survey backgrounds, etc.), you need to have S3-compatible file storage set up and connected to Continium.

Continium supports multiple storage providers (among many other S3-compatible storages):
...
Please make sure to set up a storage bucket with one of these solutions and then link it to Continium using the following environment variables:

```
  S3_ACCESS_KEY: your-access-key
  S3_SECRET_KEY: your-secret-key
  S3_REGION: us-east-1
  S3_BUCKET_NAME: continium-uploads
...
**Critical Step**: Create a complete database backup before proceeding. Continium 4.0 will automatically remove unused database tables and fields during startup.

```bash
docker exec continium-postgres-1 pg_dump -Fc -U postgres -d continium > continium_pre_v4.0_$(date +%Y%m%d_%H%M%S).dump
```
...
# Start with Continium 4.0
docker compose up -d
```

**3. Automatic Database Migration**

When you start Continium 4.0 for the first time, it will **automatically**:
...
<Info>
  With Continium 3.0, we introduced an automatic data migration system. If you're using a version older than
...
This guide is for users who are **self-hosting Continium** using the **one-click setup**. If you have a different setup, you may need to adjust the commands accordingly.
...
```bash
docker exec continium-postgres-1 pg_dump -Fc -U postgres -d continium > continium_pre_v3.3_$(date +%Y%m%d_%H%M%S).dump
```

<Info>
  If you run into “**No such container**”, use `docker ps` to find your container name,
  e.g. `continium_postgres_1`.
</Info>

If you prefer storing the backup as an `*.sql` file remove the `-Fc` (custom format) option. In case of a restore scenario you will need to use `psql` then with an empty `continium` database.

1. Pull the latest version of Continium:

```bash
docker compose pull
```

1. Stop the running Continium instance & remove the related containers:

```bash
docker compose down
```

1. Restarting the containers with the latest version of Continium:

```bash
docker compose up -d
```

When you start the latest version of **Continium**, it will **automatically detect and apply any required data migrations** during startup. You don’t need to run any manual migration steps or download separate migration images.

- Access your updated instance

Once the containers are running, simply go to the **same URL** as before to access your updated **Continium** instance.

That’s it! This new process ensures your **Continium** setup stays up to date with the latest schema changes as soon as you run the updated container.
...
```bash
docker exec continium-postgres-1 pg_dump -Fc -U postgres -d continium > continium_pre_v3.2_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker exec continium-postgres-1 pg_dump -Fc -U postgres -d continium > continium_pre_v3.1_$(date +%Y%m%d_%H%M%S).dump
```
...
With **Continium 3.0**, we're making changes to ensure long-term sustainability while still supporting open source. While the **Community Edition** has gained [new features](https://continium.com/blog/continium-3-0), some [advanced capabilities](https://continium.com/docs/self-hosting/license) are now part of the **Enterprise Edition**.

⚠️ **No Downgrade Option:** If you upgrade to **3.0** and run the data migration, **you cannot revert to 2.7.2**. If you rely on **SSO, user identification, or cluster support**, either **stay on version 2.7.x** or reach out to us on [**GitHub Discussions**](https://github.com/continium/continium/discussions) **for a custom quote**.

</Warning>

This major release brings a better approach to **data migrations**.

- If you're using **Continium v2.7 or later**, you can upgrade **directly** to **v3.x** and future versions.

- If you're on a version **older than v2.7**, you **must upgrade step-by-step** (e.g. **2.4 → 2.5 → 2.6 → 2.7**) before moving to v3.x.

### Steps to Migrate

This guide is for users **self-hosting** Continium with the **one-click setup**. If you're using a different setup, you might adjust the commands.

- &#x20;Navigate to the Continium Directory
...
```bash
docker exec continium-postgres-1 pg_dump -Fc -U postgres -d continium > continium_pre_v3.0_$(date +%Y%m%d_%H%M%S).dump
...
<Info>
  ...
  to restore it later, use `psql` with an empty Continium database.
</Info>

- Pull the latest version of Continium:

```bash
docker compose pull
```

- Stop the running Continium instance & remove the related containers:

```bash
docker compose down
```

- Restarting the containers with the latest version of Continium:

```bash
docker compose up -d
```

When you start the latest version of Continium, it will automatically detect and run any necessary data migrations during startup. There is no need to run any manual migration steps or pull any separate migration images.

- Access your updated instance

That’s it! Once the containers are up and running, simply navigate to the same URL as before to access your fully migrated Continium instance.

## v2.7

<Info>
  AI Features & Database Update: This release lays the groundwork for upcoming AI features, now in private beta. Continium requires the `pgvector` extension in PostgreSQL.

One-Click Setup Users: Use the `pgvector/pgvector:pg15` image instead of `postgres:15-alpine` to ensure compatibility.

</Info>

Continium v2.7 includes all the features and improvements developed by the community during Hacktoberfest 2024. Additionally, this release introduces an advanced team-based access control system, which is available in the Continium Enterprise Edition.
...
This guide is for users who are self-hosting Continium using our one-click setup. If you are using a different setup, you might adjust the commands accordingly.

To run all these steps, please navigate to the Continium folder where your `docker-compose.yml` file is located.
...
```bash
docker exec continium-postgres-1 pg_dump -Fc -U postgres -d continium > continium_pre_v2.7_$(date +%Y%m%d_%H%M%S).dump
```
...
- Stop the running Continium instance and remove the related containers:
...
```bash
docker pull ghcr.io/continium/data-migrations:latest && \
docker run --rm \
  --network=continium_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/continium?schema=public" \
  -e UPGRADE_TO_VERSION="v2.7" \
  ghcr.io/continium/data-migrations:v2.7.0
...
- That's it! Once the migration is finished, you can **access your Continium instance** at the same URL as before.

## v2.6

Continium v2.6 introduces advanced logic jumps for surveys, allowing more complex branching logic, including variables, conditions, and more. This release also includes numerous bug fixes, significant performance improvements for website and app surveys, and enhanced stability.
...
This guide is for users who are self-hosting Continium using our one-click setup. If you're using a different setup, you might need to adjust the commands accordingly.
...
```bash
docker exec continium-postgres-1 pg_dump -Fc -U postgres -d continium > continium_pre_v2.6_$(date +%Y%m%d_%H%M%S).dump
```
...
<Info>
  ...
  restore scenario, you will need to use `psql` then with an empty `continium` database.
</Info>
...
docker run --rm \
  ...
  ghcr.io/continium/data-migrations:v2.6.0
```
...
- Once the migration is complete, **you can access your Continium instance** at the same URL as before.

## v2.5

Continium v2.5 lets you visualise responses in a data table format. This release also brings a few bug fixes and performance improvements.
...
This guide is for users self-hosting Continium with our one-click setup. If you're using a different setup, you may need to adjust the commands.
...
```bash
docker exec continium-postgres-1 pg_dump -Fc -U postgres -d continium > continium_pre_v2.5_$(date +%Y%m%d_%H%M%S).dump
```
...
docker run --rm \
  ...
  ghcr.io/continium/data-migrations:v2.5.3
```
...
Continium v2.4 lets you create multiple endings for your surveys and choose which one users see based on logic jumps. This version also includes bug fixes and performance improvements.
...
<Info>
  ...
  triggers, but they will no longer be stored on the server to improve Continium' overall performance.
</Info>

### Steps to Migrate

This guide is for users who are self-hosting Continium with our one-click setup. If you're using a different setup, you may need to adjust the commands accordingly.
...
```bash
docker exec continium-postgres-1 pg_dump -Fc -U postgres -d continium > continium_pre_v2.4_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker pull ghcr.io/continium/data-migrations:v2.4.3 && \
docker run --rm \
  --network=continium_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/continium?schema=public" \
  -e UPGRADE_TO_VERSION="v2.4" \
  ghcr.io/continium/data-migrations:v2.4.3
```
...
- That's it! Once the migration is done, you can access your Continium instance at the same URL as before.
...
Continium v2.3 introduces new colour options for rating questions, enhanced multi-language support for Chinese (Simplified & Traditional), and includes several bug fixes and performance improvements.
...
This guide is for users self-hosting Continium with our one-click setup. If you're using a different setup, adjust the commands as needed.
...
```bash
docker exec continium-postgres-1 pg_dump -Fc -U postgres -d continium > continium_pre_v2.3_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker pull ghcr.io/continium/data-migrations:v2.3.0 && \
docker run --rm \
  --network=continium_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/continium?schema=public" \
  -e UPGRADE_TO_VERSION="v2.3" \
  ghcr.io/continium/data-migrations:v2.3.0
...
The feature to create short URLs in Continium is now deprecated. Existing short URLs will continue to work for now, but it is recommended to use long URLs instead. Support for short URL redirects will be removed in a future release.

## v2.2

Continium v2.2 introduces XM research presets, providing a brand-new product onboarding experience. Our goal is to make user research "obviously easy" across different industries, starting with Software-as-a-Service and E-Commerce.
...
```bash
docker exec continium-postgres-1 pg_dump -Fc -U postgres -d continium > continium_pre_v2.2_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker pull ghcr.io/continium/data-migrations:v2.2 && \
docker run --rm \
  --network=continium_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/continium?schema=public" \
  -e UPGRADE_TO_VERSION="v2.2" \
  ghcr.io/continium/data-migrations:v2.2
```
...
Continium v2.1 introduces more options for creating No-Code Actions and improves the self-hosting experience with a new onboarding process for fresh instances.
...
This guide is for users self-hosting Continium using our one-click setup. If you are using a different setup, you may need to adjust the commands accordingly.

To follow these steps, please navigate to the Continium folder where your `docker-compose.yml` file is located.
...
```bash
docker exec continium-postgres-1 pg_dump -Fc -U postgres -d continium > continium_pre_v2.1_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker pull ghcr.io/continium/data-migrations:v2.1.0 && \
docker run --rm \
  --network=continium_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/continium?schema=public" \
  -e UPGRADE_TO_VERSION="v2.1" \
  ghcr.io/continium/data-migrations:v2.1.0
```

Once the migration script runs successfully, your data will be migrated to the latest schema. The script is designed to run safely multiple times, so you don’t need to worry if it’s executed more than once. After completing the migration, you can continue to access your Continium instance at the same URL as before.

- That's it! Once the migration is complete, you can **now access your Continium instance** at the same URL as before.
...
Continium v2.0 introduces significant features such as **Multi-Language Surveys** and **Advanced Styling for Surveys**, aimed at improving the flexibility and appearance of your surveys. Additionally, the release includes several optimisations, bug fixes, and smaller improvements to ensure a smoother user experience.

If you are upgrading to Continium v2.0, follow this guide to ensure a smooth migration of your existing Continium instance without encountering issues or build errors.

<Info>
  This upgrade requires a **data migration**. Please make sure to back up your database before proceeding.
  Follow the steps below to upgrade your Continium instance to v2.0.
</Info>

<Info>
  If you've used the Continium Enterprise Edition with a free beta license key, your instance will be
  downgraded to the Community Edition 2.0. You can find all license details on the [license
  page](https://continium.com/docs/self-hosting/license).
</Info>
...
<Warning>
  ...
  continium/continium:latest` to `image:ghcr.io/continium/continium:latest` in
...
```bash
docker exec continium-postgres-1 pg_dump -Fc -U postgres -d continium > continium_pre_v2.0_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker pull ghcr.io/continium/data-migrations:v2.0.3 && \
docker run --rm \
  --network=continium_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/continium?schema=public" \
  -e UPGRADE_TO_VERSION="v2.0" \
  ghcr.io/continium/data-migrations:v2.0.3
```
...
```ts
import continium from "@continium/js/website";
...
```ts
import continium from "@continium/js/app";
...
Continium v1.6 introduces key features like Advanced Targeting, Segmentation, on-the-fly survey triggers, and stability improvements. This update also changes some environment variables. Follow this guide to upgrade your Continium instance to v1.6 smoothly.

<Info>
  This upgrade requires a data migration. Ensure you back up your database before proceeding. Follow the steps
  below to upgrade your Continium instance to v1.6.
</Info>

### Steps to Migrate

This guide is for users self-hosting Continium with our one-click setup. If you're using a different setup, adjust the commands accordingly.

Navigate to the Continium folder containing your `docker-compose.yml` file to run the steps.

- **Backup your Database**: This is a crucial step. Please make sure to backup your database before proceeding with the upgrade. You can use the following command to backup your database:

```bash
docker exec continium-quickstart-postgres-1 pg_dump -Fc -U postgres -d continium > continium_pre_v1.6_$(date +%Y%m%d_%H%M%S).dump
```
...
- Restarting the containers will automatically pull the latest version of Continium:
...
docker run --rm \
  ...
  ghcr.io/continium/data-migrations:v1.6.1

```

The above command will migrate your data to the latest schema. This is a crucial step to migrate your existing data to the new structure. Only if the script runs successful, changes are made to the database. The script can safely run multiple times.

1. That's it! Once the migration is complete, you can **now access your Continium instance** at the same URL as before.

#### Restoring the database after a failed upgrade

```bash
docker exec -i continium-quickstart-postgres-1 pg_restore --clean -U postgres -v -d continium < continium_pre_v1.6_<timestamp_of_your_dump_file>.dump
```

<Info>
  Replace the path to `continium_pre_v1.6_<timestamp_of_your_dump_file>.dump` with the exact path to your `.dump` file.
</Info>
...
If you're using the `@continium/js` package, update it to version \~1.6.5 to access the latest features and improvements.

<Info>
  Currently, the package needs to be pinned to `~1.6.5`, see [this
  issue](https://github.com/continium/continium/issues/2273).
</Info>

### Upgrade and pin the client package

```bash
npm install @continium/js@~1.6.5
```
...
Continium v1.2 introduces new features for Link Surveys and enhances security. However, there are some breaking changes related to environment variables. This guide will help you upgrade your Continium instance to v1.2 smoothly.
...
Continium v1.1 introduces new features and improvements but includes breaking changes with environment variables. This guide will help you upgrade your Continium instance to v1.1 without data loss.
...
| `NEXT_PUBLIC_IS_CONTINIUM_CLOUD`         | `IS_CONTINIUM_CLOUD`         |
...
x-environment: &environment
  environment:
    # The url of your Continium instance used in the admin panel
    WEBAPP_URL:

    # Required for next-auth. Should be the same as WEBAPP_URL
    NEXTAUTH_URL:

    # PostgreSQL DB for Continium to connect to
    DATABASE_URL: "postgresql://postgres:postgres@postgres:5432/continium?schema=public"
...
If you have any questions or require help, feel free to reach out to us on [**GitHub Discussions**](https://github.com/continium/continium/discussions). 😃[
](https://continium.com/docs/developer-docs/rest-api)
