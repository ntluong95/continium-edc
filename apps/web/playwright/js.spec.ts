...
const HTML_TEMPLATE = `<head>
  <script type="text/javascript">
    !(function () {
      ...
      (t.type = "text/javascript"), (t.async = !0), (t.src = "http://localhost:3000/js/continium.umd.cjs");
      var e = document.getElementsByTagName("script")[0];
      t.onload = function(){
        if (window.continium) {
          window.continium.setup({environmentId: "ENVIRONMENT_ID", appUrl: "http://localhost:3000"});
        } else {
          console.error("Continium library failed to load properly. The continium object is not available.");
        }
      };
...
<body style="background-color: #fff">
  <p>This is my sample page using the Continium JS javascript widget</p>
</body>
`;
...
test.describe("JS Package Test", async () => {
  ...
  test("Create, display and validate PMF survey", async ({ page, users }) => {
    ...
    await expect(page.locator("#continium-modal-container")).toHaveCount(1);
    await expect(
      page.locator("#questionCard-0").getByRole("link", { name: "Powered by Continium" })
    ).toBeVisible();
