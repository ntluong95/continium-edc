...
test.describe("Survey overview", () => {
  test("loads surveys, applies filters and sort, and paginates with load more", async ({ page, users }) => {
    ...
    await user.login();
    await page.waitForURL(/\/environments\/[^/]+\/forms/);
    const environmentId =
      /\/environments\/([^/]+)\/forms/.exec(page.url())?.[1] ??
      (() => {
    ...
    await page.getByPlaceholder("Search by form name").fill(targetSurveyName);
  ...
  test("shows preview and copy link for published link surveys and hides the dropdown when no actions are available", async ({
    ...
    await user.login();
    await page.goto(`/environments/${environmentId}/forms`);
