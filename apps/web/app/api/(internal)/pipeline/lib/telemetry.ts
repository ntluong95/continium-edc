import { createCacheKey, getCacheService } from "@continium/cache";
...
/**
 * Sends telemetry events to Continium Enterprise endpoint.
...
/**
 * Gathers telemetry data and sends it to Continium Enterprise endpoint.
...
const sendTelemetry = async (lastSent: number) => {
  ...
  // Instead of many parallel count queries (which could exhaust the connection pool),
  // we batch all count queries into a single raw SQL query.
  const [countsResult, ssoProviders] = await Promise.all([
  ...
  const payload = {
    ...
    sso: ssoMap,
    meta: {
      version: packageJson.version, // Continium version for compatibility tracking
    },
    temporal: {
  ...
  // Send telemetry to Continium Enterprise endpoint.
  // This endpoint collects usage statistics for enterprise license validation and analytics.
  const url = `https://ee.continium.com/api/v1/instances/${instanceId}/usage-updates`;
