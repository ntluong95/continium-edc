  ...
  try {
    if (authentication) {
      if ("user" in authentication) {
        // Session-based authentication for routes that accept user sessions
 ...
 *   - authentication: Authentication result (type determined by route)
 * @param params.action - Optional audit action type (e.g., "created", "updated", "deleted"). Required for audit logging
 * @param params.targetType - Optional audit target type (e.g., "survey", "response"). Required for audit logging
