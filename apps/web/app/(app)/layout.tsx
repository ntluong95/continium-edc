...
import { ContiniumEntitlementsProvider } from "@/modules/continium/licensing/components/continium-entitlements-provider";
...
const AppLayout = async ({ children }: { children: React.ReactNode }) => {
  ...
  const continiumEntitlements = await getContiniumEntitlements();
  ...
  return (
    <NextAuthProvider sessionMaxAge={SESSION_MAX_AGE}>
      ...
      <ToasterClient />
      <ContiniumEntitlementsProvider value={continiumEntitlements}>{children}</ContiniumEntitlementsProvider>
    </NextAuthProvider>
  );
};
