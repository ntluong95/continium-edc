...
import FBLogo from "@/images/continium-wordmark.svg";
...
interface NavigationProps {
  ...
  project: { id: string; name: string; kind: "PRODUCT" | "CLINICAL" };
...
export const MainNavigation = ({
  ...
  const mainNavigation = useMemo(
    () => [
      ...
      // Clinical section — only visible when project is in CLINICAL mode
      {
        name: "Protocol",
        href: `/environments/${environment.id}/clinical/protocol`,
        icon: ClipboardListIcon,
        isActive: pathname?.includes("/clinical/protocol"),
        isHidden: project.kind !== "CLINICAL",
        disabled: isMembershipPending || isBilling,
      },
      {
        name: "Subjects",
        href: `/environments/${environment.id}/clinical/subjects`,
        icon: UsersIcon,
        isActive: pathname?.includes("/clinical/subjects"),
        isHidden: project.kind !== "CLINICAL",
        disabled: isMembershipPending || isBilling,
      },
      {
        name: "DAGs",
        href: `/environments/${environment.id}/clinical/dags`,
        icon: ShieldIcon,
        isActive: pathname?.includes("/clinical/dags"),
        isHidden: project.kind !== "CLINICAL",
        disabled: isMembershipPending || isBilling,
      },
      {
        name: "Audit Trail",
        href: `/environments/${environment.id}/clinical/audit`,
        icon: HistoryIcon,
        isActive: pathname?.includes("/clinical/audit"),
        isHidden: project.kind !== "CLINICAL",
        disabled: isMembershipPending || isBilling,
      },
    ],
    [t, environment.id, pathname, isMembershipPending, isBilling, project.kind]
  );
  ...
  const dropdownNavigation = [
    ...
    },
    {
      ...
      href: "https://continium.com/docs",
      target: "_blank",
      icon: ArrowUpRightIcon,
    },
    {
      label: t("common.share_feedback"),
      href: "https://github.com/continium/continium/issues",
      target: "_blank",
      icon: ArrowUpRightIcon,
    },
  ];
  ...
  const organizationSettings = [
    ...
    },
    {
      ...
      hidden: isContiniumCloud || isMember,
    },
  ];
  ...
  const trialDaysRemaining = useMemo(() => {
    if (!isContiniumCloud || organization.billing?.stripe?.subscriptionStatus !== "trialing") return null;
  ...
  const mainNavigationLink = isBilling
    ? getBillingFallbackPath(environment.id, isContiniumCloud)
    : `/environments/${environment.id}/forms/`;

  const handleProjectChange = (projectId: string) => {
    const targetPath =
      projectId === project.id ? `/environments/${environment.id}/forms` : `/workspaces/${projectId}/`;
    startTransition(() => {
    ...
    return [
      {
        ...
        href: isLicenseActive
          ...
          : "https://continium.com/upgrade-self-hosted-license",
      },
      {
        text: t("common.cancel"),
        onClick: () => setOpenProjectLimitModal(false),
      ...
      {project && (
        <aside
            ...
            <div className="flex items-center justify-between px-3 pb-4">
              {!isCollapsed && (
                <Link
                  ...
                  <Image src={FBLogo} width={160} height={30} alt={t("environments.continium_logo")} />
                </Link>
              )}
              <Button
                variant="ghost"
          ...
          <div>
            ...
            {!isCollapsed && isOwnerOrManager && latestVersion && !isContiniumCloud && !isDevelopment && (
              <Link
                href="https://github.com/continium/continium/releases"
                target="_blank"
            ...
            {!isCollapsed && isContiniumCloud && trialDaysRemaining !== null && (
