...
import { SuccessMessage } from "@/app/(app)/environments/[environmentId]/forms/[surveyId]/(analysis)/summary/components/SuccessMessage";
import { ShareSurveyModal } from "@/app/(app)/environments/[environmentId]/forms/[surveyId]/(analysis)/summary/components/share-survey-modal";
import { SurveyStatusDropdown } from "@/app/(app)/environments/[environmentId]/forms/[surveyId]/components/SurveyStatusDropdown";
...
export const SurveyAnalysisCTA = ({
  ...
  const duplicateSurveyAndRoute = async (surveyId: string) => {
    ...
    if (duplicatedSurveyResponse?.data) {
      ...
      router.push(`/environments/${environment.id}/forms/${duplicatedSurveyResponse.data.id}/edit`);
    } else {
    ...
    },
    {
      ...
      onClick: () => {
        responseCount > 0
          ...
          : router.push(`/environments/${environment.id}/forms/${survey.id}/edit`);
      },
      isVisible: !isReadOnly,
    },
  ];
