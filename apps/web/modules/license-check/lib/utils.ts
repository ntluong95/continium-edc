import "server-only";
import { AUDIT_LOG_ENABLED, IS_CONTINIUM_CLOUD, IS_RECAPTCHA_CONFIGURED } from "@/lib/constants";
...
export const getOrganizationProjectsLimit = async (organizationId: string): Promise<number> => {
  ...
  if (entitlementsContext.licenseStatus === "active") {
    // null means unlimited (enterprise); a number caps at that value
    return entitlementsContext.licenseFeatures?.projects ?? Infinity;
  }
