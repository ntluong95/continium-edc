...
vi.mock("@/lib/env", () => ({
  env: {
    ...
    CONTINIUM_LICENSE_SERVER_URL: "https://licenses.continium.com",
    VERCEL_URL: "some.vercel.url",
    CONTINIUM_COM_URL: "https://app.continium.com",
...
vi.mock("@/lib/constants", async (importOriginal) => {
  ...
  return {
    ...
    IS_CONTINIUM_CLOUD: false, // Default to self-hosted for most tests
...
describe("License Core Logic", () => {
  ...
  describe("Environment-based endpoint selection", () => {
    test("should use CONTINIUM_LICENSE_SERVER_URL for endpoint", async () => {
      vi.resetModules();
      vi.doMock("@/lib/env", () => ({
        env: {
          ENTERPRISE_LICENSE_KEY: "test-license-key",
          ENVIRONMENT: "staging",
          CONTINIUM_LICENSE_SERVER_URL: "https://licenses-staging.continium.com",
          HTTPS_PROXY: undefined,
          HTTP_PROXY: undefined,
        },
      }));
      ...
      // Verify CONTINIUM_LICENSE_SERVER_URL controls the endpoint
      expect(fetch).toHaveBeenCalledWith(
        "https://licenses-staging.continium.com/api/licenses/check",
        expect.objectContaining({
          method: "POST",
