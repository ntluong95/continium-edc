...
import { validateConfirmationName } from "@/modules/projects/settings/general/lib/delete-project-confirmation";
    ...
    if (deleteProjectResponse?.data) {
      if (organizationProjects.length === 1) {
        ...
        if (productionEnvironment) {
          localStorage.setItem(CONTINIUM_ENVIRONMENT_ID_LS, productionEnvironment.id);
        }
      }
  ...
  return (
    <div>
      ...
      <DeleteDialog
        ...
        disabled={!validateConfirmationName(confirmationName, currentProject.name)}>
        <Input
          value={confirmationName}
          onChange={(e) => setConfirmationName(e.target.value)}
          placeholder={t("common.delete_workspace_confirmation_name")}
        />
      </DeleteDialog>
    </div>
  );
