    ...
    handler: async ({ authentication, parsedInput: { query, params } }) => {
      if (IS_CONTINIUM_CLOUD) {
        return handleApiError(request, {
          type: "bad_request",
          details: [{ field: "organizationId", issue: "This endpoint is not supported on Continium Cloud" }],
        });
      }
      ...
      if (IS_CONTINIUM_CLOUD) {
        return handleApiError(
          request,
          {
            type: "bad_request",
            details: [
              { field: "organizationId", issue: "This endpoint is not supported on Continium Cloud" },
            ],
          },
          auditLog
        );
