...
import { TTemplate, TTemplateFilter, TTemplateRole } from "@continium/types/templates";
...
export const TemplateTags = ({ template, selectedFilter }: TemplateTagsProps) => {
  ...
  const getIndustryTag = (industries: TProjectConfigIndustry[] | undefined): string | undefined => {
    // if user selects an industry e.g. clinicalTrial than the tag should not say "Multiple industries" anymore but "E-Commerce".
    ...
    return industries.length > 1
      ? t("environments.surveys.templates.multiple_designs")
