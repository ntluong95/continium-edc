...
import { publishSurveyAsClinicalInstrument } from "@/modules/clinical/instruments/lib/publish-survey-as-clinical-instrument";
...
export const updateSurveyDraftAction = authenticatedActionClient.inputSchema(ZSurveyDraft).action(
  withAuditLogging("updated", "survey", async ({ ctx, parsedInput }) => {
    ...
    revalidatePath(`/environments/${result.environmentId}/forms/${result.id}`);
  ...
  withAuditLogging("updated", "survey", async ({ ctx, parsedInput }) => {
    ...
    if (isPublish) {
      await publishSurveyAsClinicalInstrument({
        survey: { id: result.id, name: result.name, blocks: result.blocks },
        projectId,
        actorId: ctx.user.id,
      });
      revalidatePath(`/environments/${result.environmentId}/forms`);
      revalidatePath(`/environments/${result.environmentId}/clinical/instruments`);
      revalidatePath(`/environments/${result.environmentId}/clinical/protocol`);
    }
    ...
    return results.map(
      (result: {
        ...
        return {
          ...
          urls: {
            regularWithAttribution: `${result.urls.regular}&dpr=2&authorLink=${authorLink}&authorName=${authorName}&utm_source=continium&utm_medium=referral`,
