...
import { TSurvey, TSurveyWelcomeCard } from "@continium/types/surveys/types";
...
vi.mock("@continium/types/surveys/validation", () => ({
...
describe("Metadata Utils", () => {
  ...
  describe("getBasicSurveyMetadata", () => {
    ...
    test("adds Continium to title when IS_CONTINIUM_CLOUD is true", async () => {
      // Temporarily modify the mocked module
      vi.doMock("@/lib/constants", () => ({
        IS_CONTINIUM_CLOUD: true,
        WEBAPP_URL: "https://test.continium.com",
      }));
      ...
      expect(result.title).toBe("Test Survey | Continium");
    ...
    test("handles welcome card headline with HTML content", async () => {
      const { getTextContent } = await import("@continium/types/surveys/validation");
