...
export const WebhookSettingsTab = ({
  ...
  const handleTestEndpoint = async (sendSuccessToast: boolean): Promise<boolean> => {
    try {
      ...
      const result = await testEndpointAction({ url: testEndpointInput, webhookId: webhook.id });
  ...
  const onSubmit = async (data: {
    ...
    const result = await updateWebhookAction({ webhookId: webhook.id, webhookInput: updatedData });
    if (result?.data) {
      router.refresh();
    ...
    <div>
      <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="col-span-1">
          ...
          <div className="mt-1 flex">
            <Input
              {...register("url", { value: testEndpointInput })}
              type="text"
      ...
      <DeleteDialog
        ...
        onDelete={async () => {
          setOpen(false);
          const result = await deleteWebhookAction({ id: webhook.id });
          if (result?.data) {
            router.refresh();
