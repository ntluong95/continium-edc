...
const renderSelectedTriggersText = (webhook: Webhook, t: TFunction) => {
  ...
  const cleanedTriggers = webhook.triggers.map((trigger) => {
    if (trigger === "responseCreated") return t("environments.integrations.webhooks.response_created");
    if (trigger === "responseUpdated") return t("environments.integrations.webhooks.response_updated");
    if (trigger === "responseFinished") return t("environments.integrations.webhooks.response_finished");
    return trigger;
  });
