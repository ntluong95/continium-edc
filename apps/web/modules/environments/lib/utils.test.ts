...
import { TMembership } from "@continium/types/memberships";
...
vi.mock("@continium/types/errors", () => ({
...
describe("utils.ts", () => {
  ...
  describe("getEnvironmentLayoutData", () => {
    ...
    test("fetches cloud metrics when IS_CONTINIUM_CLOUD is true", async () => {
      // Mock IS_CONTINIUM_CLOUD to be true
      const constantsMock = await import("@/lib/constants");
      vi.mocked(constantsMock).IS_CONTINIUM_CLOUD = true;
