...
export const LoginForm = ({
  ...
  const onSubmit: SubmitHandler<TLoginForm> = async (data) => {
    if (typeof window !== "undefined") {
      localStorage.setItem(CONTINIUM_LOGGED_IN_WITH_LS, "Email");
    }
    try {
        ...
        <div className="space-y-2">
          ...
          {(isSsoEnabled || !totpLogin) && (
            <SSOOptions
              ...
              source="signin"
              showPrimaryOptionsWhenDisabled
            />
          )}
        </div>

        {(publicSignUpEnabled || inviteToken) && !totpLogin && (
          <div className="mt-9 text-center text-xs">
            <span className="leading-5 text-slate-500">{t("auth.login.new_to_continium")}</span>
            <br />
            <Link
