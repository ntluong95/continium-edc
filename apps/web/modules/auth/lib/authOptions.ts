...
import type { TUser } from "@continium/types/user";
import {
  CONTROL_HASH,
...
import { ACCOUNT_DELETION_SSO_REAUTH_CALLBACK_PATH } from "@/modules/account/constants";
...
import { getConfiguredSSOProviders, getSSOProviders } from "@/modules/ee/sso/lib/providers";
import { handleSsoCallback } from "@/modules/ee/sso/lib/sso-handlers";
import { auditUserLogin, auditUserLoginFailed, auditUserLogout } from "./audit-auth-events";
...
export const authOptions: NextAuthOptions = {
  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials, _req) {
        const auditFailedLogin = (reason: string, userId?: string | null) => {
          void auditUserLoginFailed({
            userId,
            reason,
        ...
        if (!credentials) {
          ...
          auditFailedLogin("no_credentials");
        ...
        if (credentials.password && credentials.password.length > 128) {
          ...
          auditFailedLogin("password_too_long");
          throw new Error("Invalid credentials");
        }
        ...
        if (!isValid) {
          ...
          auditFailedLogin("invalid_password", user.id);
          throw new Error("Invalid credentials");
        }
        ...
        if (user.twoFactorEnabled && credentials.backupCode) {
          if (!ENCRYPTION_KEY) {
            ...
            auditFailedLogin("encryption_key_missing", user.id);
            throw new Error("Internal Server Error");
          }

          if (!user.backupCodes) {
            logTwoFactorAttempt(false, "backup_code", user.id, user.email, "no_backup_codes");
            auditFailedLogin("no_backup_codes", user.id);
          ...
          try {
            ...
            auditFailedLogin("invalid_backup_codes", user.id);
          ...
          if (index === -1) {
            ...
            auditFailedLogin("invalid_backup_code", user.id);
          ...
          if (secret.length !== 32) {
            ...
            auditFailedLogin("invalid_2fa_secret", user.id);
          ...
          if (!isValidToken) {
            ...
            auditFailedLogin("invalid_totp_code", user.id);
    ...
    ...(ENTERPRISE_LICENSE_KEY ? getSSOProviders() : getConfiguredSSOProviders()),
  ],
  session: {
    strategy: "database",
  ...
  callbacks: {
    ...
    async signIn({ user, account }) {
      ...
      // When this is an account-deletion SSO re-authentication the callbackUrl points at
      // the dedicated completion route. Skip the normal SSO provisioning path entirely —
      // the completion route performs all identity checks and deletes the account.
      const isDeletionReauth =
        callbackUrl.includes(ACCOUNT_DELETION_SSO_REAUTH_CALLBACK_PATH) && account !== null;
      if (isDeletionReauth) {
        await finalizeSuccessfulSignIn({
          userId,
          email: userEmail,
  ...
  events: {
    // Fire-and-forget: audit failures must never block the auth flow.
    async signIn({ user }) {
      void auditUserLogin({ userId: user.id });
    },
    async signOut(message) {
      // message.session is present for database sessions; .token for JWT.
      const userId =
        "session" in message && message.session
          ? ((message.session as { userId?: string; user?: { id?: string } }).userId ??
            (message.session as { user?: { id?: string } }).user?.id)
          : undefined;
      void auditUserLogout({ userId });
    },
  },
  pages: {
    signIn: "/auth/login",
