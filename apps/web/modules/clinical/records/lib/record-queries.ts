...
import { PermissionDeniedError, assertCanReadInstrument } from "@/modules/clinical/access/lib/assert-helpers";
...
export interface TDataEntryInstrument {
  ...
  instrument: {
    ...
    survey:
      | ({ id: string; name: string; type: string; status: SurveyStatus } & Record<string, unknown>)
      | null;
...
export const getSubjectDataEntry = async (
  ...
  const loadDataEntry = async (): Promise<TSubjectDataEntry | null> => {
    ...
    const initialEventId =
      eventId && visibleEvents.some((event) => event.id === eventId)
        ? eventId
