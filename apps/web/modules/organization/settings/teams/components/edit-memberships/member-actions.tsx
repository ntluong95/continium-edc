...
import { TMember } from "@continium/types/memberships";
  ...
  const handleResendInvite = async () => {
    try {
      ...
      if (resendInviteResponse?.data) {
        if (resendInviteResponse.data.emailSent) {
          toast.success(t("environments.settings.general.invitation_sent_once_more"));
        } else if (resendInviteResponse.data.inviteToken) {
          setShareInviteToken(resendInviteResponse.data.inviteToken);
          setShowShareInviteModal(true);
          toast.success("Invite refreshed. Email delivery is not configured, so share the invite link.");
        }
        router.refresh();
      } else {
