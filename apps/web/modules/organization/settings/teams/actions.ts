...
import { ZId, ZUuid } from "@continium/types/common";
import { AuthenticationError, OperationNotAllowedError, ValidationError } from "@continium/types/errors";
import { TOrganizationRole, ZOrganizationRole } from "@continium/types/memberships";
import { INVITE_DISABLED, IS_CONTINIUM_CLOUD } from "@/lib/constants";
...
export const resendInviteAction = authenticatedActionClient.inputSchema(ZResendInviteAction).action(
  withAuditLogging("updated", "invite", async ({ ctx, parsedInput }) => {
    ...
    let emailSent = false;
    try {
      emailSent = await sendInviteMemberEmail(
    ...
    const inviteToken = !emailSent
      ? encodeURIComponent(
          createInviteToken(parsedInput.inviteId, updatedInvite.email, {
            expiresIn: "7d",
          })
        )
      : undefined;

    return { ...updatedInvite, emailSent, inviteToken };
  })
);
...
export const inviteUserAction = authenticatedActionClient.inputSchema(ZInviteUserAction).action(
  withAuditLogging("created", "invite", async ({ ctx, parsedInput }) => {
    ...
    if (!IS_CONTINIUM_CLOUD && parsedInput.role === OrganizationRole.billing) {
    ...
    const inviteToken = !emailSent
      ? encodeURIComponent(
          createInviteToken(inviteId, parsedInput.email, {
            expiresIn: "7d",
          })
        )
      : undefined;

    return { inviteId, emailSent, inviteToken };
  })
);
