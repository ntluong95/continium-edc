...
import { ZMembershipUpdateInput } from "@continium/types/memberships";
import { IS_CONTINIUM_CLOUD, USER_MANAGEMENT_MINIMUM_ROLE } from "@/lib/constants";
...
export const updateInviteAction = authenticatedActionClient.inputSchema(ZUpdateInviteAction).action(
  withAuditLogging("updated", "invite", async ({ ctx, parsedInput }) => {
    ...
    if (!IS_CONTINIUM_CLOUD && parsedInput.data.role === "billing") {
