...
import { TUserEmail, ZUserEmail } from "@continium/types/user";
import { validateInputs } from "@/lib/utils/validate";

export type TMailingListId = "security" | "product-updates";

const MAILING_LIST_ENDPOINTS: Record<TMailingListId, string> = {
  security: "https://ee.continium.com/api/v1/public/mailing/security/subscriptions",
  "product-updates": "https://ee.continium.com/api/v1/public/mailing/product-updates/subscriptions",
} as const;
...
export const subscribeUserToMailingList = async ({
  ...
  if (isContiniumCloud && subscribeToProductUpdates) {
    await subscribeToMailingList({ email, listId: "product-updates" });
  } else if (!isContiniumCloud && subscribeToSecurityUpdates) {
