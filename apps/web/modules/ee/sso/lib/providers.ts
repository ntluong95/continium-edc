...
export const getConfiguredSSOProviders = () =>
  getSSOProviders().filter((provider) => {
    switch (provider.id) {
      case "github":
      ...
      case "azure-ad":
        return Boolean(AZUREAD_CLIENT_ID && AZUREAD_CLIENT_SECRET);
      case "openid":
        return Boolean(OIDC_CLIENT_ID && OIDC_CLIENT_SECRET && OIDC_ISSUER);
      case "saml":
        return false;
      default:
        return false;
