...
export const EditBranding = ({ type, isEnabled, projectId, isReadOnly }: EditBrandingProps) => {
  ...
  const toggleBranding = async () => {
    ...
    if (updateBrandingResponse?.data) {
      toast.success(
        newBrandingState
          ? t("environments.workspace.look.continium_branding_shown")
          : t("environments.workspace.look.continium_branding_hidden")
      );
    } else {
  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      <Label htmlFor={`branding-${type}`}>
        {t("environments.workspace.look.show_continium_branding_in", {
