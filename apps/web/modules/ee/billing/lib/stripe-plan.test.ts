...
describe("stripe-plan", () => {
  test("maps known product metadata values to cloud plans", () => {
    expect(
      getCloudPlanFromProduct(product({ id: "prod_hobby", metadata: { continium_plan: "hobby" } }))
    ).toBe("hobby");
    expect(getCloudPlanFromProduct(product({ id: "prod_pro", metadata: { continium_plan: "pro" } }))).toBe(
      "pro"
    );
    expect(
      getCloudPlanFromProduct(product({ id: "prod_scale", metadata: { continium_plan: "scale" } }))
    ).toBe("scale");
    expect(
      getCloudPlanFromProduct(product({ id: "prod_custom", metadata: { continium_plan: "custom" } }))
    ).toBe("custom");
  });
  ...
  test("falls back to unknown for missing or unknown products", () => {
    ...
    expect(
      getCloudPlanFromProduct(product({ id: "prod_unknown", metadata: { continium_plan: "enterprise" } }))
    ).toBe("unknown");
  });
});
