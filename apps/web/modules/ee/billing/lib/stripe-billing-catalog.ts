...
import type { TCloudBillingInterval } from "@continium/types/organizations";
...
const getPricePlan = (price: Stripe.Price): TStandardCloudPlan | null => {
  ...
  const plan =
    price.metadata?.continium_plan ??
    (!product || product.deleted ? undefined : product.metadata?.continium_plan);
...
const getPriceInterval = (price: Stripe.Price): TCloudBillingInterval | null => {
  const metadataInterval = normalizeInterval(price.metadata?.continium_interval);
  if (metadataInterval) {
...
const getPriceKind = (price: Stripe.Price): TStripePriceKind | null => {
  const metadataKind = price.metadata?.continium_price_kind;
