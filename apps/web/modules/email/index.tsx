...
} from "@continium/email";
import { TEmailTemplateLegalProps } from "@continium/email/src/types/email";
import { logger } from "@continium/logger";
import type { TLinkSurveyEmailData } from "@continium/types/email";
import { InvalidInputError, ResourceNotFoundError } from "@continium/types/errors";
...
import { TUserEmail, TUserLocale } from "@continium/types/user";
import {
  DEBUG,
  IMPRINT_ADDRESS,
  IMPRINT_URL,
...
export const sendEmail = async (emailData: SendEmailDataProps): Promise<boolean> => {
  ...
  try {
    ...
    const emailDefaults = {
      from: `${MAIL_FROM_NAME ?? "Continium"} <${MAIL_FROM ?? "noreply@continium.com"}>`,
    };
