...
export default defineConfig({
  test: {
    ...
    // *.integration.test.ts is reserved for tests that touch the database,
    // long-running scenarios, or wider-than-unit setups. They are run by
    // `pnpm test:integration` against a real DB; the default `pnpm test`
    // run skips them so unit feedback stays fast.
