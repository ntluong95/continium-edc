...
import { TIntegrationItem } from "@continium/types/integration";
import {
...
} from "@continium/types/integration/airtable";
...
export const fetchAirtableAuthToken = async (formData: Record<string, unknown>) => {
...
async function waitForFieldsToExist(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    if (attempt < maxRetries) {
      logger.error(
        `Attempt ${attempt}/${maxRetries}: ${missingFields.length} field(s) still missing [${missingFields.join(", ")}], retrying in ${intervalMs / 1000}s…`
      );
  ...
  throw new Error(
    `Timed out waiting for ${missingFields.length} field(s) [${missingFields.join(", ")}] to become available. Available fields: [${Array.from(existingFields).join(", ")}]`
  );
}
