...
import { ZId, ZOptionalNumber, ZString } from "@continium/types/common";
...
import { TUserNotificationSettings } from "@continium/types/user";
import { IS_CONTINIUM_CLOUD, ITEMS_PER_PAGE } from "@/lib/constants";
...
const getDefaultOrganizationBilling = (): TOrganizationBilling => ({
  limits: {
    projects: IS_CONTINIUM_CLOUD ? 1 : 3,
    monthly: {
      responses: IS_CONTINIUM_CLOUD ? 250 : 1500,
    },
  },
...
export const deleteOrganization = async (organizationId: string) => {
  ...
  try {
    ...
    if (IS_CONTINIUM_CLOUD && stripeCustomerId) {
