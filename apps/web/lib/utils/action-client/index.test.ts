...
import { ContiniumFeatureDisabledError } from "@continium/licensing";
import {
  AuthenticationError,
  AuthorizationError,
...
vi.mock("@continium/logger", () => ({
  logger: {
    withContext: () => ({ error: vi.fn(), info: vi.fn() }),
    warn: vi.fn(),
  },
}));
...
describe("actionClient handleServerError", () => {
  ...
  describe("expected errors should NOT be reported to Sentry", () => {
    ...
    test("ContiniumFeatureDisabledError returns generic message (NOT the feature key) and is not sent to Sentry", async () => {
      const result = await executeThrowingAction(new ContiniumFeatureDisabledError("clinicalExports"));
      expect(result?.serverError).toBe("Feature is not enabled");
      expect(result?.serverError).not.toContain("clinicalExports");
      expect(Sentry.captureException).not.toHaveBeenCalled();
    });
  });
