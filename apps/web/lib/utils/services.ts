...
export const getWebhook = reactCache(
  async (webhookId: string): Promise<{ environmentId: string } | null> => {
    validateInputs([webhookId, ZId]);
    try {
...
export const getIntegration = reactCache(
  async (integrationId: string): Promise<{ environmentId: string } | null> => {
    validateInputs([integrationId, ZId]);
    try {
