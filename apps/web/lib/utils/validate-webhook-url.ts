...
import { InvalidInputError } from "@continium/types/errors";
...
const PRIVATE_IPV6_PREFIXES = [
  "::1", // Loopback
  "fe80:", // Link-local
  "fc", // Unique local address (ULA, fc00::/7)
  "fd", // Unique local address (ULA, fc00::/7)
];

const isPrivateIPv4 = (ip: string): boolean => PRIVATE_IPV4_PATTERNS.some((pattern) => pattern.test(ip));
  ...
  new Promise((resolve, reject) => {
    ...
    dns.resolve(hostname, (errV4, ipv4Addresses) => {
      ...
      dns.resolve6(hostname, (errV6, ipv6Addresses) => {
        const ipv6: ResolvedAddress[] = errV6
          ? []
          : ipv6Addresses.map((ip) => ({ ip, family: 6 as const }));
        const all = [...ipv4, ...ipv6];
        if (all.length === 0) {
...
/**
 * Validates a webhook URL to prevent SSRF.
 * Thin wrapper around {@link validateAndResolveWebhookUrl} for callers that only need validation.
 *
 * @throws {InvalidInputError} when the URL fails any validation check
 */
 ...
 * closing the DNS-rebinding TOCTOU window between URL validation and HTTP delivery.
 */
